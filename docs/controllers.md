...
- `AbstractOAuthInitiateController` — state-CSRF + redirect to provider.
  ...
  For a provider marked `FormPostOAuthProviderInterface` (cross-site `form_post` callback) the state — and, for a link, the initiating user — is also written to a dedicated `SameSite=None; Secure; HttpOnly`, **HMAC-signed** single-use cookie; both this controller and `AbstractOAuthCallbackController` therefore take a **required** `StateCookieSignerInterface` constructor argument (registered with `$secret: '%kernel.secret%'`, `$clock: '@clock'` and a `$ttl`), which signs the cookie so its contents — crucially the link-initiating user — can't be forged by a client crafting the request directly, and stamps a signed expiry so a captured value stops working once the `$ttl` has passed rather than remaining valid indefinitely. Also pass the optional `Security` constructor argument so the link case can capture the logged-in user.
- `AbstractOAuthCallbackController` — fetch user info, then branch login / link / auto-register.
  `getOAuthGroup`, `getCallbackRouteName`, `getFirewallName`, `getStateSessionKey`, `getIntentSessionKey`, `getConfirmPendingSessionKey`, `getLoginRoute`, `getDashboardUrl`, `getSocialAccountsRoute`, `getConfirmLinkRoute`, `getAuditChannel`, `getAuditUserIdKey`, `isAcceptableCurrentUser(?$user)`, `findExistingLinkUser($info): ?UserInterface`, `findUserByEmail($email): ?UserInterface`, `findUserByIdentifier($identifier): ?UserInterface`, `canAutoRegister($info): bool`, `registerAndLink($info): UserInterface`, `linkExistingUser($user, $info)`, `touchLastUsed($user, $info)`, `handlePostLogin($user, $request)`.
  `findUserByIdentifier` recovers the link-initiating user on a cross-site `form_post` callback, where the session (and so the logged-in user) is unavailable — typically `$userProvider->loadUserByIdentifier($identifier)`.
  **`findExistingLinkUser` must resolve the link by `($info->getProvider(), $info->getProviderUserId())` — never by `$info->getEmail()`.** A hit here is the "already linked" branch, which signs the user in immediately with no ownership proof; the email-matched branch deliberately routes through `AbstractOAuthConfirmLinkController` instead. Keying this lookup on the email would collapse that distinction and turn a forged email claim into a full sign-in — and on Apple / Microsoft the email carries no `email_verified` attestation at all (see [OAuth social login](features/oauth-social-login.md#the-three-callback-outcomes)). Use the provider's immutable subject id (Google `sub`, Microsoft `oid`, Apple `sub`), which is what `getProviderUserId()` already returns.
