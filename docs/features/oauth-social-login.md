...
- `AutoRegistrationPolicy` (`AutoRegistrationPolicyInterface`) — `canAutoRegister($info, ?array $allowedEmailDomains)`. Requires a non-empty email that the provider did not report as **explicitly unverified**, then: `null` ⇒ allow any such email; `[]` ⇒ deny (auto-registration off); a list ⇒ allow only those email domains.
  > **`email_verified` is three-state, and only `false` rejects.** The policy refuses on `isEmailVerified() === false` and lets `null` ("unknown") through — treating silence as failure would disable auto-registration on every provider that emits no such claim. Of the bundled providers **only `GoogleOAuthProvider` populates the flag**; `AppleOAuthProvider` and `MicrosoftOAuthProvider` always pass `null`, so on those two the policy's email check reduces to "non-empty". What that means per provider:
  >
  > - **Google** — sends `email_verified`; the flag is mapped through and the policy enforces it. Trustworthy.
  > - **Apple** — attests, and `AppleAccessToken` honours it: it takes the id_token's email **only** when `email_verified` is truthy. Apple states the email is in the identity token on *every* response ("Apple provides the user's email address in the identity token on all subsequent API responses" — only the *name* is first-authorization-only, which is why `AppleOAuthProvider` reads the name from the request's `user` field). So in normal operation Apple's email is attested; the bundle simply doesn't surface the flag. Narrow exception: when the id_token carries no email at all — Apple documents this for managed Apple Accounts in Apple School Manager, where "the `email` claim may be empty" — `Apple::createResourceOwner` falls back to `$response['email']`, which `Apple::fetchResourceOwnerDetails` reads from `$_GET['user']` / `$_POST['user']`, a client-supplied value.
  > - **Microsoft** — Entra emits no `email_verified` claim. Microsoft's own claims reference warns: *"Never use `email` or `upn` claim values to store or determine whether the user in an access token should have access to data"*, and calls `email` *"not guaranteed to be correct, and mutable over time"*. The optional **`xms_edov`** ("whether the user's email domain owner has been verified") exists for exactly this purpose but is **off unless enabled in the app registration**, requires the `email` claim to be present, and is only set for Microsoft, Google and OTP-authenticated accounts — Facebook and SAML/WS-Fed federated accounts never have a verified domain. `AzureResourceOwner::toArray()` passes every claim through, so `MicrosoftOAuthProvider::buildUserInfo()` can read it without touching the vendor.
  >
  > None of this endangers an **existing** account — a matching email routes to the outcome-2 ownership challenge, never to a sign-in. It matters only where an unattested email is trusted on its own: auto-registration. **Microsoft is the case that needs attention**; see [Auto-registration safety](#settings) below.
...
Outcome 1 is keyed on the **identity**, outcomes 2 and 3 on the **email** — and that split is what keeps a forged email claim from becoming a takeover. Your `findExistingLinkUser($info)` must therefore look the link up by the `(getProvider(), getProviderUserId())` pair and **never** by `getEmail()`: outcome 1 signs the user straight in with no ownership proof, so an email-keyed lookup would hand an attacker who can claim an arbitrary email in their provider profile (see the note on `email_verified` above) an existing account without ever reaching the outcome-2 challenge. Key on the provider's immutable subject id — Google's `sub`, Microsoft's `oid`, Apple's `sub` — not on an address the profile owner can edit.
...
> **Auto-registration safety.** Pass `null` for "any email may register" (commercial signups) and `[]` to disable it; a domain list restricts it. For the **admin** scope, only ever whitelist domains you fully control — anyone with a working email in a listed domain could self-create a privileged account. Creating the account (roles, locale, …) happens in your `AbstractOAuthCallbackController` subclass, not in the bundle.
>
> **A domain whitelist is not proof of domain ownership — on Microsoft.** It filters the email *claim*, and Entra does not attest that claim: Microsoft's own guidance is *"never use it for authorization"*. With `tenant: 'common'` (or `organizations`) the endpoint accepts sign-ins from **any** Entra tenant, including one an attacker registers in minutes and populates with whatever `email` they like — a whitelisted domain included. The exposure is **accounts provisioned under addresses the signer does not own**; existing accounts stay protected by the outcome-2 ownership challenge.
>
> Mitigations, in order of preference: pin `tenant` to your own tenant id; enable the `xms_edov` optional claim and reject sign-ups where it isn't `true`; disable auto-registration (`[]`) and provision through a flow that verifies the address itself; or send your own confirmation email before the account gets anything. A domain list alone substitutes for none of these.
>
> Google and Apple both attest the address (see the `email_verified` note above), so this concerns Microsoft in practice.
...
> **`tenant` is a security boundary, not just a routing hint — pick the narrowest value you can.** `common` and `organizations` accept sign-ins from every Entra tenant, so the `email` claim becomes attacker-controllable; a tenant GUID removes that. It matters only if you auto-register — see [Auto-registration safety](#settings). The `oid` used as the identity key is unaffected and stays safe on any `tenant` value.
